...
class PNG extends ImageDecoder {
	...
	public function decode(bytes:Bytes):Void {
			...
			switch type {
				...
				case "tRNS":
					switch colorType {
						case 0:
							if (length >= 2)
								transparentGray = DecodeTools.u16BE(bytes, dataOffset);
						case 2:
							if (length >= 6)
								transparentRgb = [
		...
		var src = 0;
		final passes:Array<{x:Int, y:Int, dx:Int, dy:Int}> = interlaceMethod == 0
			? [{x: 0, y: 0, dx: 1, dy: 1}]
			: interlacePasses;
		...
		for (pass in passes) {
			...
			for (passY in 0...passHeight) {
				...
				writeScanline(current, pass.x, pass.y, pass.dx, pass.dy, passY, passWidth, bitDepth, colorType, channels, palette, paletteAlpha, transparentGray,
					transparentRgb);
		...
		for (x in 0...rowWidth) {
			...
			switch colorType {
				...
				case 2:
					...
					final alpha = transparentRgb != null && rSample == transparentRgb[0] && gSample == transparentRgb[1] && bSample == transparentRgb[2] ? 0 : 255;
				...
				case 6:
					writeRgba(dst,
						sampleToByte(readSample(line, bitDepth, x, 4, 0), bitDepth),
						sampleToByte(readSample(line, bitDepth, x, 4, 1), bitDepth),
						sampleToByte(readSample(line, bitDepth, x, 4, 2), bitDepth),
						sampleToByte(readSample(line, bitDepth, x, 4, 3), bitDepth));
				case _:
			}
		}
	}
