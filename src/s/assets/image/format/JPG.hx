...
class JPG extends ImageDecoder {
	...
	public function new(asset) {
		super(asset);
		components = Vector.fromArrayCopy([
			makeComponent(),
			makeComponent(),
			makeComponent()
		]);
		quantTables = Vector.fromArrayCopy([
			new Vector(64),
			new Vector(64),
			new Vector(64),
			new Vector(64)
	...
	function colIDCT(base:Int, output:Bytes, outputPos:Int, stride:Int):Void {
		...
		x2 = (181 * (x4 + x5) + 128) >> 8;
		x4 = (181 * (x4 - x5) + 128) >> 8;
		out[outputPos] = clip(((x7 + x1) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x3 + x2) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x0 + x4) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x8 + x6) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x8 - x6) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x0 - x4) >> 14) + 128); outputPos += stride;
		out[outputPos] = clip(((x3 - x2) >> 14) + 128); outputPos += stride;
	...
	function upsampleH(component:JpegComponent):Void {
		...
		for (_ in 0...component.height) {
			...
			for (x in 0...xmax) {
				dst[dstPos + (x << 1) + 3] = clipFilter(cf4a * src[srcPos + x] + cf4b * src[srcPos + x + 1] + cf4c * src[srcPos + x + 2] + cf4d * src[srcPos + x + 3]);
				dst[dstPos + (x << 1) + 4] = clipFilter(cf4d * src[srcPos + x] + cf4c * src[srcPos + x + 1] + cf4b * src[srcPos + x + 2] + cf4a * src[srcPos + x + 3]);
			}
